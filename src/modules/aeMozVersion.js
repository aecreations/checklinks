...
export let aeMozVersion = {
  ...
  getMozVersion(aVersion)
  {
    ...
    else if (parsedVer.releaseType == this.RELEASETYPE_DEVEL) {
      let devBuildSfx = '';
      if (aeConst?.DEV_BUILD) {
        devBuildSfx = "+";
      }
      rv = `${aVersion}${devBuildSfx}`;
    }
    else {
      let devBuildSfx = '';
      if (aeConst?.DEV_BUILD) {
  ...
  getExtendedVersion(aVersion)
  {
    ...
    if (parsedVer.releaseType == this.RELEASETYPE_STABLE) {
      ...
      if (parsedVer.releaseType == this.RELEASETYPE_DEVEL) {
        // Development release versions have the major digit of 0 (zero).
        // Treat them as stable version numbers.
        rv = mozVer;
      }
      else {
        rv = `${mozVer} (${aVersion})`;
  ...
  _parse(aVersion)
  {
    ...
    if (major == 0) {
      rv.releaseType = this.RELEASETYPE_DEVEL;
    }
    else if ([98, 99, 998, 999].includes(Number(minor))) {

...
export class aeDialog
{
  ...
  _init()
  {
    ...
    let backdropElt = document.querySelector("#lightbox-bkgrd-ovl");
    if (!backdropElt) {
      backdropElt = document.createElement("div");
      backdropElt.id = "lightbox-bkgrd-ovl";
      backdropElt.className = "lightbox-bkgrd";

      let lightboxElts = document.querySelectorAll(".lightbox");
      let lastLightboxElt = lightboxElts[lightboxElts.length - 1];
      lastLightboxElt.insertAdjacentElement("afterend", backdropElt);
    }
